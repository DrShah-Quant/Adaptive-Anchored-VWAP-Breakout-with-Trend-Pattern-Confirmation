...
class AlgoEvent:
    ...
    def massive_upward_momentum(self):
        labels = [label for _, label, _ in reversed(self.trend_labels)]
        i = 0
        ...
        downCount = 0
        while i < len(labels) and labels[i] in [0, -1]:
            downCount += 1
            i += 1
        ...
        if firstUP > 0 and lastUP > 0 and downCount > 0:
            ratio1 = downCount / firstUP
            ratio2 = lastUP / downCount
            if ratio1 >= 0.25 and 1 <= ratio2 <= 1.2:
                return True
    ...
    def check_buy_pattern(self, lows, highs, current_price, current_volume):
        ...
        if self.highest_low != 0 and self.highest_high != 0 and current_price < self.highest_low:
            price_range = abs(highs[-1][0] - lows[-1][0])
            
            if self.sellFlag == False:
                for tradeID, buy_price, _ in self.successful_buys:
                    if buy_price < current_price:
                        res = self.evt.update_opened_order(tradeID, current_price )
                        self.sellFlag = True
                        self.evt.consoleLog(res["status"], res["msg"]) 
            ...
            if anchored_vwap and current_price > 1.05 * anchored_vwap:
              ...
              #      if (0.70 * slope1 < slope2 and l1[1] < h1[1] < l2[1] < h2[1] < l3[1]) or self.massive_upward_momentum():
              #  self.evt.consoleLog(f"---ABOVE ANCHOR UPTREND------------")
                ...
                if self.successful_buys:
                    ...
                    #else:
                    #    capital_to_use = 0.10 * self.available_capital
                    #    self.evt.consoleLog(f"--10%-- (Avg Buy Price: {avg_price:.4f})")
                ...
                self.evt.consoleLog(f"Above Anchor Volume to buy: {volume_to_buy}")
                ...
                order.takeProfitLevel = current_price * 1.08
                ...
                if volume_to_buy > 0:
                    ...
                    self.evt.consoleLog(f"STOP ORDER Above Anchor Uptrend: {volume_to_buy:.0f} units @ {current_price}, Stop Loss: {stop_loss:.2f}")
        
                return False
                    #else: 
                    #    for tradeID, buy_price, volume in self.successful_buys:
                            #if buy_price * 1.05 < current_price:
                    #        order = AlgoAPIUtil.OrderObject()
                    #        order.orderRef = f"close_{tradeID}"
                    #        order.tradeID = tradeID
                    #        order.openclose = 'close'
                    #        order.volume = volume
                    #        self.evt.consoleLog(f"CLOSING TRADE Weak Above ANCHOR: TradeID={tradeID}, Volume={volume}, BuyPrice={buy_price}, CurrentPrice={current_price}")
                     #       self.evt.sendOrder(order)
                #elif self.massive_upward_momentum():
                #    avg_price = 0
                #    capital_to_use = 0
            ...
            if len(lows) >= 3 and len(highs) >= 2:
                ...
                if (0.85 * slope1 < slope2 and l1[1] < h1[1] < l2[1] < h2[1] < l3[1]) or self.massive_upward_momentum() :
                    self.evt.consoleLog(f"---WITHIN BAND UPTREND------------")
                    return True

        # Fallback if not enough lows/highs but current price > avg price
        
        if  (self.highest_low == 0 and  self.highest_high == 0 and current_price > avg_price) or self.massive_upward_momentum():
            ...
            order.price = current_price
            order.volume = volume_to_buy
            order.takeProfitLevel = current_price * 3
            order.stopLossLevel = stop_loss
            order.timeinforce = self.timeinforce
            if volume_to_buy > 0:
                self.evt.sendOrder(order)
            ...
            if slope1 * 0.70 > slope2:
                ...
                for tradeID, buy_price, volume in self.successful_buys:
                    if current_price > round(1.08 * buy_price, 4):
                        ...
                        order.openclose = 'close'
                        order.volume = volume
                        self.evt.consoleLog(f"CLOSING TRADE The Highs Weak: TradeID={tradeID}, Volume={volume}, BuyPrice={buy_price}, CurrentPrice={current_price}")
                        self.evt.sendOrder(order)
                if should_close:
                    return True
        return False
    ...
    def can_buy_with_volume(self, current_price, current_volume, lows, highs, open_price, high_price, low_price):
        ...
        if self.successful_buys:
            avg_price = round(sum(price for _, price, _ in self.successful_buys) / len(self.successful_buys), 4)
            if current_price * 1.03 < avg_price:
                capital_to_use = 0.60 * self.available_capital
                self.evt.consoleLog(f"--60%-- (Avg Buy Price: {avg_price:.4f})")
            else:
                capital_to_use = 0.50 * self.available_capital
                self.evt.consoleLog(f"--50%-- (Avg Buy Price: {avg_price:.4f})")
